        ...
        <div className="flex min-h-screen bg-white dark:bg-dark-bg">
            ...
            <main className="flex-1 w-full lg:pl-[280px] pt-24 pb-12">
                <section className="px-6 md:px-12 max-w-[1600px] mx-auto">
                    ...
                    <BlurFade delay={0.1}>
                        <div className="mb-16 mt-12">
                            ...
                            <h1 className="text-6xl md:text-7xl font-serif-display italic text-dark-text dark:text-dark-text-primary leading-[0.9] tracking-tight mb-6">
                                <AnimatedGradientText className="text-6xl md:text-7xl font-serif-display italic">
                                    Thời Trang
                    ...
                    {/* Masonry Grid with MagicCards */}
                    <div className="columns-1 md:columns-2 lg:columns-3 gap-8 space-y-8">
                        {products.map((product, idx) => (
                            <BlurFade key={product.id} delay={0.1 + idx * 0.05}>
                                <Link
                                    href={`/shop/${product.id}`}
                                    className={`block break-inside-avoid group relative cursor-pointer ${idx % 3 === 1 ? 'md:pt-12' : ''}`}
                                >
                                    <MagicCard className="overflow-hidden bg-off-white dark:bg-dark-card border-0">
                                        ...
                                        {product.isBestSeller && (
                                            <div className="absolute top-4 left-4 z-20">
                                                <span className="inline-block px-3 py-1 bg-primary/90 text-[10px] font-bold uppercase tracking-widest backdrop-blur-sm text-white">Bán Chạy</span>
                                            </div>
                                        )}
                                    ...
                                    <div className="mt-4 flex justify-between items-start opacity-70 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <span className="font-mono text-xs text-primary font-bold">${product.price}</span>
                                    </div>
                                </Link>
                            </BlurFade>
                        ))}

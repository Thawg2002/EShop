...
import { Minus, Plus, ShoppingBag, ArrowRight, Trash2, Heart, ShieldCheck } from 'lucide-react';
        ...
        return (
            <div className="min-h-screen pt-40 pb-20 flex flex-col items-center justify-center space-y-8 container mx-auto px-6 text-center">
                ...
                <div className="space-y-4">
                    <h1 className="text-4xl font-bold">Giỏ hàng của bạn đang trống</h1>
                    <p className="text-muted-foreground max-w-sm mx-auto">Hãy khám phá các sản phẩm tuyệt vời của chúng tôi và thêm chúng vào giỏ hàng.</p>
                </div>
                <Link
                    href="/cua-hang"
                    className="bg-primary text-white px-10 py-4 rounded-full font-bold uppercase tracking-widest text-xs"
                >
                    Quay lại cửa hàng
                </Link>
            </div>
            ...
            <div className="container mx-auto px-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                    <div className="lg:col-span-8 space-y-8">
                        <div className="hidden md:grid grid-cols-12 gap-4 pb-4 border-b border-border text-[10px] uppercase font-bold tracking-widest text-muted-foreground">
                                ...
                                {items.map((item) => (
                                    ...
                                    >
                                        <div className="col-span-1 md:col-span-6 flex items-center space-x-6">
                                            ...
                                            <div className="space-y-1">
                                                <h3 className="text-lg font-bold hover:text-primary transition-colors">
                                                    <Link href={`/${item.product.slug || item.product._id}`}>{item.product.name}</Link>
                                                </h3>
                                                {item.variant && (
                                                ...
                                                <div className="flex items-center space-x-4 pt-2">
                                                    <button className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground hover:text-primary transition-colors flex items-center">
                                        ...
                                        <div className="hidden md:block col-span-2 text-right">
                                            <p className="text-sm font-medium text-muted-foreground">{formatCurrency(item.price)}</p>
                                        </div>
                    ...
                    <div className="lg:col-span-4">
                        <div className="bg-muted/30 rounded-3xl p-8 sticky top-32 space-y-8 border border-border/50">
                            <h4 className="text-xl font-bold">Tổng đơn hàng</h4>
                            ...
                            <Link
                                href="/thanh-toan"
                                className="w-full bg-primary text-white py-5 rounded-full font-bold uppercase tracking-widest text-xs flex items-center justify-center space-x-3 shadow-lg transition-all hover:-translate-y-1 active:scale-95"
                            >
